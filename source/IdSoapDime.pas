...
type
  ...
  public
    ...
    property TypeInfo : Ansistring read FTypeInfo write FTypeInfo;
  ...
  private
    ...
    function GetItemByName(AName: ansistring): TIdSoapDimeRecord;
  ...
  public
    ...
    property ItemByName[AName : ansistring] : TIdSoapDimeRecord read GetItemByName;
    function Add(AId : Ansistring = ''):TIdSoapDimeRecord;
...
begin
  ...
  StreamWriteBinary(AStream, @AString[1], Length(AString) {$IFDEF UNICODE}*2{$ENDIF});
end;
...
begin
  ...
  StreamReadBinary(AStream, @Result[1], Length(Result){$IFDEF UNICODE}*2{$ENDIF});
end;
...
procedure TIdSoapDimeRecord.SetContent(const AValue: TStream);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapDimeRecord.SetContent';
begin
  assert(self.TestValid(TIdSoapDimeRecord), ASSERT_LOCATION+': self is not valid');
  // Can't check AStream
...
function TIdSoapDimeMessage.Add(AId: Ansistring): TIdSoapDimeRecord;
...
function TIdSoapDimeMessage.GetItemByName(AName: ansistring): TIdSoapDimeRecord;
