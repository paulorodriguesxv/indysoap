...
type
  ...
  // comms focused - for client app to track status of server
  TIdClientStartEvent = procedure (AClient : TIdSoapBaseSender) of object;
  TIdClientStopEvent = procedure (AClient : TIdSoapBaseSender; AException : Exception) of object;
  ...
  TIdSoapBaseSender = class(TIdSoapITIProvider {$IFDEF VER130}, system.IUnknown {$ENDIF})
  Private
    ...
    FOnClientStart : TIdClientStartEvent;
    FOnClientStop  : TIdClientStopEvent;
    ...
    function  Execute(AInterface : TIdSoapITIInterface; AMethod : TIdSoapITIMethod; AParams: Pointer) : Int64;
    function  CommonEntry(AGlue: TIdSoapInterfaceGlue; ACallID: Integer; AParams: Pointer): Int64;
    ...
    function  ProcessOutParams(ABaseNode: TIdSoapNode; AData: Pointer; ASenderContext: TIdSoapSenderContext; AParamIndex: Integer; AParam: TIdSoapITIParameter; ASoapReader: TIdSoapReader; AIsParameter : boolean) : Boolean;
    function  ProcessOutInteger(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String) : boolean;
    function  ProcessOutFloat(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String) : boolean;
    function  ProcessOutEnumeration(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String; ATypeInfo: PTypeInfo; AParam : TIdSoapITIParameter) : boolean;
    function  ProcessOutSet(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String; AParamType : PTypeInfo; AParam : TIdSoapITIParameter) : boolean;
    function  ProcessOutStringChar(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String; ATypeInfo: PTypeInfo) : boolean;
    function  ProcessOutDynArray(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String; ATypeInfo: PTypeInfo; ASenderContext: TIdSoapSenderContext; AParam : TIdSoapITIParameter) : boolean;
    function  ProcessOutClass(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String; ATypeInfo: PTypeInfo; ASenderContext: TIdSoapSenderContext; AParamFlag: TParamFlag; AParam : TIdSoapITIParameter) : boolean;
  ...
  Protected
    ...
    function  QueryInterface(const AIID: {$IFDEF VER130} System. {$ENDIF} TGUID; out AObj): HResult; Override; Stdcall;
  ...
  Public
    ...
    function SoapSend(AWriter : TIdSoapWriter; AInterface : TIdSoapITIInterface; ASoapAction : string = '') : TIdSoapReader;
  ...
  Published
    ...
    Property OnClientStart : TIdClientStartEvent read FOnClientStart write FOnClientStart;
    Property OnClientStop  : TIdClientStopEvent read FOnClientStop write FOnClientStop;
  end;
...
begin
  ...
  FStubBuffer := FPool.Acquire(sizeof(TIdSoapBuffer), xpid);
  FStubBuffer^.FNext := NIL;
  FStubBuffer^.FExecId := xpid;
...
begin
  ...
  if (FOffset + ABytes) > ID_SOAP_BUFFER_SIZE then  // not enough space at end of buffer
    begin
    LBuf := FPool.Acquire(sizeof(TIdSoapBuffer), xpid);
    FCurrentBuffer^.FNext := LBuf;
    FCurrentBuffer^.FExecId := xpid;
...
begin                   // can probably be replaced with a faster search later
  ...
  for LIndex := 0 to FInterfaces.Count - 1 do
    begin
    s1 := GUIDToString(AGUID);
    s2 := GUIDToString(PIdSoapInterfaceInfo(FInterfaces[LIndex])^.FGUID);
...
begin
  ...
  try
    for LIndex := 0 to AITI.Interfaces.Count - 1 do
      begin
      LInterface := AITI.Interfaces.IFace[LIndex];
...
End;
{$ELSE}
Procedure TIdSoapBaseSender.InitComponent;
Begin
  inherited;
  Init;
End;
...
Procedure TIdSoapBaseSender.Init;
begin
...
function TIdSoapBaseSender.ProcessOutParams(ABaseNode: TIdSoapNode; AData: Pointer; ASenderContext: TIdSoapSenderContext; AParamIndex: Integer; AParam: TIdSoapITIParameter; ASoapReader: TIdSoapReader; AIsParameter : boolean): boolean;
...
begin
  ...
  case AParam.TypeInformation^.Kind of
    tkInt64,
    tkInteger:     result := ProcessOutInteger(LBasicType,LData^,ASoapReader,ABaseNode,LName);
    tkFloat:       result := ProcessOutFloat(LBasicType,LData^,ASoapReader,ABaseNode,LName);
    tkEnumeration: result := ProcessOutEnumeration(LBasicType,LData^,ASoapReader,ABaseNode,LName,AParam.TypeInformation, AParam);
    tkSet:         result := ProcessOutSet(LBasicType,LData^,ASoapReader,ABaseNode,LName, AParam.TypeInformation, AParam);
    {$IFDEF UNICODE}
    tkUString,
    {$ENDIF}
    tkLString,
    ...
    tkString,
    tkChar,
    tkWChar:       result := ProcessOutStringChar(LBasicType,LData^,ASoapReader,ABaseNode,LName,AParam.TypeInformation);
    tkDynArray:    result := ProcessOutDynArray(LBasicType,LData^,ASoapReader,ABaseNode,LName,AParam.TypeInformation,ASenderContext, AParam);
    tkClass:       result := ProcessOutClass(LBasicType,LData^,ASoapReader,ABaseNode,LName,AParam.TypeInformation,ASenderContext,AParam.ParamFlag, AParam);
...
function TIdSoapBaseSender.ProcessOutInteger(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String) : Boolean;
...
begin
  ...
  result := not (seoUseDefaults in EncodingOptions) or ASoapReader.ParamExists[ABaseNode, AParamName];
  if result then
    begin
    case ABasicType of
...
function TIdSoapBaseSender.ProcessOutFloat(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String) : Boolean;
...
function TIdSoapBaseSender.ProcessOutEnumeration(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String; ATypeInfo: PTypeInfo; AParam : TIdSoapITIParameter) : boolean;
...
function TIdSoapBaseSender.ProcessOutSet(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String; AParamType : PTypeInfo; AParam : TIdSoapITIParameter) : Boolean;
...
function TIdSoapBaseSender.ProcessOutStringChar(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String; ATypeInfo: PTypeInfo) : boolean;
...
function TIdSoapBaseSender.ProcessOutDynArray(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String; ATypeInfo: PTypeInfo; ASenderContext: TIdSoapSenderContext; AParam : TIdSoapITIParameter) : boolean;
...
function TIdSoapBaseSender.ProcessOutClass(ABasicType: TIdSoapBasicType; Var AData; ASoapReader: TIdSoapReader; ABaseNode: TIdSoapNode; AParamName: String; ATypeInfo: PTypeInfo; ASenderContext: TIdSoapSenderContext; AParamFlag: TParamFlag; AParam : TIdSoapITIParameter) : Boolean;
  ...
  if Assigned(LSpecialType) then
    ...
    if Assigned(ABaseNode) then
      ...
      if not LPrecreated then
        ...
        Assert(assigned(LClassType), ASSERT_LOCATION+': Reference to class "'+string(LClassName)+'" could not be resolved');
      ...
      try
        ...
        for LSlot := 1 to LPropMan.Count do
          ...
          case LPropType^.Kind of
            tkInteger:
              begin
              if ProcessOutParams(LRootNode,LBuf,ASenderContext,-1,LFakeParam,ASoapReader, false) then
                begin
            ...
            tkUString:
              begin
              ProcessOutParams(LRootNode,@LUnicodeString,ASenderContext,-1,LFakeParam,ASoapReader,false);
              LPropMan.AsString[LClass,LSlot] := LUnicodeString;
              end;
            {$ENDIF}
            tkWString:
              begin
...
begin
  ...
  case ABasicType of
    ...
    isbtEnumShortInt:
        begin
        if {no default} (ADefault = MAXINT) or
           {see comment above} (ADefault = 0) or
           {we send whether default or not} (not (seoSendNoDefaults in EncodingOptions) or
           {value is not same as default} (ShortInt(AData) = ADefault)) then
          begin
    ...
    isbtEnumSmallInt:
        begin
        if (ADefault = MAXINT) or (ADefault = 0) or (not (seoSendNoDefaults in EncodingOptions) or (SmallInt(AData) <> ADefault)) then
          begin
    ...
    isbtEnumWord:
        begin
        if (ADefault = MAXINT) or (ADefault = 0) or (not (seoSendNoDefaults in EncodingOptions) or (Word(AData) <> ADefault)) then
          begin
    ...
    isbtEnumInteger:
        begin
        if (ADefault = MAXINT) or (ADefault = 0) or (not (seoSendNoDefaults in EncodingOptions) or (Integer(AData) <> ADefault)) then
          begin
...
begin
  ...
  LSpecialType := IdSoapSpecialType(String(ATypeInfo^.Name));
  if Assigned(LSpecialType) then
    begin
    try
    ...
    if Assigned(LRoot) then
      ...
      Assert(Assigned(LPropMan), ASSERT_LOCATION+'["'+Name+'"]: Unable to locate property info for class ' + String(LWorkingType^.Name));
      for LIndex:=1 to LPropMan.Count do
        ...
        case LPropType^.Kind of
          tkInteger:
            ...
            ProcessParameter(LRoot,LName,ASenderContext,ASoapWriter,LBuf^,AParam,LPropMan[LIndex]^.PropType^,-1,
                              false, LPropMan.DefaultValue[LClass,LIndex]);
            end;
          tkFloat:
            begin
            case GetTypeData(LPropType)^.FloatType of
          ...
          tkUString:
            begin
            LUnicodeString := LPropMan.AsString[LClass,LIndex];
            ProcessParameter(LRoot,LName,ASenderContext,ASoapWriter,LUnicodeString,AParam,LPropMan[LIndex]^.PropType^,-1, false);
            end;
          {$ENDIF}
          tkWString:
            begin
        ...
        LRoot.ParamIsAttribute[LName] := TIdBaseSoapableClass(LClass).IsAttribute(LPropMan[LIndex]^.Name);
        end;
      end; // Assigned(LRoot)
    end;
end;
...
begin
  ...
  case AResultType^.Kind of
    ...
    else              raise EIdSoapUnknownType.Create(ASSERT_LOCATION+'["'+Name+'"]: '+RS_ERR_ENGINE_UNKNOWN_TYPE+'('+String(AResultType^.Name)+'/'+inttostr(ord(AResultType^.Kind))+')');
    end;
end;
...
begin
  ...
  AMethod.ReplaceTypeName(string(AResultType^.Name), DefaultNamespace, LType, LTypeNS);
...
function TIdSoapBaseSender.SoapSend(AWriter : TIdSoapWriter; AInterface : TIdSoapITIInterface; ASoapAction : string = '') : TIdSoapReader;
...
begin
  ...
  {$IFDEF ID_SOAP_SHOW_NODES}
  ShowNode('Client Params',AInterface.Name,AMethod.Name,LSoapWriter.BaseNode);
  {$ENDIF}
  ...
  try
    ...
    if FHasResponses then
      ...
      try
        if assigned(FOnClientStart) then
          FOnClientStart(self);
        if assigned(FOnClientStop) then
          begin
          try
            DoSoapRequest(ASoapAction, LRequestMimeType, LRequest, LResponse, LResponseMimeType);
            FOnClientStop(self, nil);
          except
            on e:exception do
              begin
              FOnClientStop(self, e);
              raise;
              end;
          end;
          end
        ...
        try
          ...
          {$IFDEF ID_SOAP_SHOW_NODES}
          ShowNode('Client Ret/Outs/Vars',AInterface.Name,AMethod.Name,LSoapReader.BaseNode);
          {$ENDIF}
        except
          on e:exception do
            begin
...
function TIdSoapBaseSender.CommonEntry(AGlue: TIdSoapInterfaceGlue; ACallID: Integer; AParams: Pointer): Int64;
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapBaseSender.CommonEntry';
var
  AMethod: TIdSoapITIMethod;
begin
  if AGlue.FParent.FSender = nil then
    begin
    Assert(ACallId=3, ASSERT_LOCATION+': Invalid interface method call. Client has previously been destroyed'); // don't use name, isn't valid if test fails
    AGlue._Release;
    result := 0;
    exit;
    end;
  ...
  Result := 0;
  if ACallId <= 3 then  // stops wasting time doing multiple mostly redundent tests
    begin
    if ACallId = 1 then       // its QueryInterface  (but its for the SOAP object, not the client)
      begin
      if not IsEqualGUID(AGlue.IID,TGUID(AParams^)) then  // its not us
        Result := E_NOTIMPL
      else
        begin
        inc(PansiChar(AParams), Sizeof(TGUID));
        Pointer(AParams^) := @AGlue.FInterfacePtr;  // assign to OUT (2nd param in QueryInterface )
        end;
      end
    ...
    else if ACallID = 2 then  // its an addref
      begin
      Result := AGlue._AddRef;
      end
    else if ACallId = 3 then  // its a release
      begin
    ...
    AMethod := AGlue.Intf.Methods.Objects[ACallID - 4] as TIdSoapITIMethod;  // interface methods start at 4 (Delphi TIndex starts at 0)
    result := Execute(AGlue.Intf, AMethod, AParams);
    end;
end;

function TIdSoapBaseSender.Execute(AInterface : TIdSoapITIInterface; AMethod : TIdSoapITIMethod; AParams: Pointer): Int64;
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapBaseSender.CommonEntry';
//type
//  PString = ^String;
var
...
begin
  ...
  if not FHasResponses then
    begin
    CheckMethodForNoResponseMode(AMethod);
    end;
  FRecvHeaders.DeleteAll;
  LSoapWriter := CreateWriter;
  try
    if AInterface.EncodingOverride <> semNotSpecified then
      begin
      LSoapWriter.EncodingMode := AInterface.EncodingOverride;
      end
    else
      begin
      LSoapWriter.EncodingMode := AMethod.EncodingMode;
      end;
    LSoapWriter.AttachmentEncoding := AInterface.AttachmentType;
    LClientContext := TIdSoapSenderContext.Create;
    try
      if AInterface.Namespace <> '' then
        begin
        LSoapWriter.SetMessageName(AMethod.RequestMessageName, AInterface.Namespace);
        end
      else
        begin
        LSoapWriter.SetMessageName(AMethod.RequestMessageName, DefaultNamespace);
        end;
      ProcessHeadersSend(AMethod, LSoapWriter, LClientContext);
      for LParamIndex := 0 to AMethod.Parameters.Count - 1 do
        begin
        LParam := AMethod.Parameters.Param[LParamIndex];
        inc(PAnsiChar(LParamPtr), ProcessParameter(nil,'',LClientContext, LSoapWriter, LParamPtr^, LParam, IdSoapGetTypeInfo(LParam.NameOfType), LParamIndex, true));
        end;
      LSoapReader := SoapSend(LSoapWriter, AInterface, GetSoapAction(AInterface, AMethod));
      if FHasResponses then
        begin
        Assert(LSoapReader.TestValid(TIdSoapReader), ASSERT_LOCATION +': SoapSend returned an invalid reader');
        try
          IdRequire((LSoapReader.MessageName = AMethod.ResponseMessageName) or (LSoapReader.MessageName = AMethod.RequestMessageName), ASSERT_LOCATION+'["'+Name+'"]: '+RS_ERR_CLIENT_WRONG_MESSAGE+' "'+AMethod.ResponseMessageName+'" / "'+LSoapReader.MessageName+'"');
          for LParamIndex := 0 to AMethod.Parameters.Count - 1 do  // look for any var,out etc param types
            begin
            LParam := AMethod.Parameters.Param[LParamIndex];
            ProcessOutParams(nil,nil,LClientContext, LParamIndex, LParam, LSoapReader, true);
            end;
          if AMethod.ResultType <> '' then  // it was a function so get the result
            begin
            FResultParamName := AMethod.ReplaceName('result', ID_SOAP_NAME_RESULT);
            ...
            LResultType := IdSoapGetTypeInfo(AMethod.ResultType);
            Result := ProcessResult(LResultType, LSoapReader, AParams, LClientContext, AMethod);
            end;
          ProcessHeadersRecv(AMethod, LSoapReader, LClientContext);
        finally
...
begin
  ...
  LNamespace := ITI.Interfaces.IFace[ITI.Interfaces.IndexOf(AInterfaceName)].Namespace;
  if LNamespace = '' then
    begin
  ...
  try
    ...
    try
      LITIDescriber.Describe(ITI.Interfaces.IFace[ITI.Interfaces.IndexOf(AInterfaceName)], GetWSDLLocation);
    finally
...
End;
{$ELSE}
Procedure TIdSoapBaseClient.InitComponent;
Begin
  inherited;
  Init;
End;
...
Procedure TIdSoapBaseClient.Init;
begin
  FHasResponses := true;
end;
...
End;
{$ELSE}
Procedure TIdSoapBaseMsgSender.InitComponent;
Begin
  inherited;
  Init;
End;
...
Procedure TIdSoapBaseMsgSender.Init;
begin
  FHasResponses := false;
end;
...
End;
{$ELSE}
Procedure TIdSoapWebClient.InitComponent;
Begin
  inherited;
  Init;
End;
{$ENDIF}
Procedure TIdSoapWebClient.Init;
begin
