...
type
  ...
  private
    ...
    function  ReadDimeWrapper(ASoapPacket : TStream; var VMimeType : String) : TStream;
    function  ReadMimeWrapper(ASoapPacket : TStream; var VMimeType : String) : TStream;

    procedure ObserveXmlEncodingType(ASource : TStream);
    function GetParameter(ANode: TIdSoapNode; const AName: String; canBeNull : boolean = false): TIdReaderParameter;
    ...
    procedure EnforceType(const AParamName: string; const AParam : TIdReaderParameter; const ANamespaceList : array of string; const ATypeList : array of string; ARoutineName: string); // clunky, but anamespace and aname iterate together
  ...
  Protected
    ...
    function GetParamRef(ANode: TIdSoapNode; const AName, AType, ATypeNS: String): String; Override;
    function GetParamIsAttribute(ANode: TIdSoapNode; const AName: String): Boolean; Override;
    function GetHasEmptyParam(ANode: TIdSoapNode; const AName: String): Boolean; Override;
  ...
  private
    ...
    procedure EncodeNode(AParent: TIdSoapXmlElement; ASoapNode: TIdSoapNode; Namespace : String);
    procedure DefineParameter(ANode: TIdSoapNode; const AName, AType, ATypeNamespace, AValue: String; AIsNil : boolean = false; AHRef : string = '');
    ...
    function  EncodeComplex(AParent: TIdSoapXmlElement; ASoapNode: TIdSoapNode; Namespace : String) : TIdSoapXmlElement;
    ...
    procedure EncodeAttachmentsDime(AStream : TStream; var VMimeType : String);
    procedure EncodeAttachmentsMime(AStream : TStream; var VMimeType : String);
  ...
  Public
    ...
    procedure DefineParamAnsiString(ANode: TIdSoapNode; const AName: String; AValue: AnsiString); Override;
    ...
    procedure DefineParamRef(ANode: TIdSoapNode; const AName, AType, ATypeNS: String; AValue: String); Override;
    procedure DefineParamIsAttribute(ANode: TIdSoapNode; const AName : String; aValue : Boolean); Override;
  end;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ObserveXmlEncodingType';
var
  LPos : Int64;
  LWord : word;
begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.CheckSoapEncodingStyle';
var
  s : string;
begin
  assert(Self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ProcessSoapException';
var
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ReadMessage';
var
  LStream : TStream;
begin
  assert(Self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': self is not valid');
  ...
  if AMimeType = ID_SOAP_HTTP_DIME_TYPE then
    begin
    LStream := ReadDimeWrapper(ASoapPacket, AMimeType);
    ASoapPacket := LStream;
    end
  else if AnsiSameText(copy(AMimeType, 1, length(ID_SOAP_MULTIPART_RELATED)), String(ID_SOAP_MULTIPART_RELATED)) then
    begin
    LStream := ReadMimeWrapper(ASoapPacket, AMimeType);
    ASoapPacket := LStream;
    end;
  try
    FDomErr := '';
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.CheckPacketOK';
var
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ProcessHeaders';
var
  LNode : TIdSoapXmlElement;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.PreDecode';
var
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.DecodeMessage';
begin
  assert(Self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ReadSimpleElement';
var
...
{  if (EncodingMode = semRPC) and (seoRequireTypes in EncodingOptions) then
  ...
  LParam.FHRef := AXmlElement.getAttribute('', ID_SOAP_NAME_XML_HREF);
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ReadComplex';
var
  LId : string;
  LIdRef : integer;
  LType : string;
...
begin
  ...
  if AXmlElement.HasNilAttribute then { do not localize }
    ...
    For i := 0 to AXmlElement.AttributeCount - 1 do
    Begin
      AXmlElement.getAttributeName(i, sNamespace, sName);
      if (sNamespace <> ID_SOAP_NS_SCHEMA_INST_2001) And (sNamespace <> ID_SOAP_NS_SCHEMA_INST_1999) And (sNamespace <> ID_SOAP_NS_XML_CORE) Then
      Begin
        LParam := TIdReaderParameter.Create;
        LParam.FXmlLink := AXmlElement;
        LParam.FValue := AXmlElement.getAttribute(sNamespace, sName);
        LParam.FIsAttribute := true;
        LChildNode.AddParam(sName, LParam);
      End;
    End;
    ...
    while assigned(LNode) do
      ...
      assert((EncodingMode = semDocument) or (not LNode.hasID), ASSERT_LOCATION+': Node on complex "'+LName+'" has an id');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.SplitWrappersRight';
var
  c, b:integer;
  LStr : string;
begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.ReadArrayNumbers';
var
  LSize : string;
  LLeft : string;
  i : integer;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.CheckMsgEncodingStyle';
begin
  assert(Self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.CheckMsgSchemaVersion';
var
  LFound : boolean;
...
begin
  ...
  for i := ANode.Children.count -1 downto 0 do
    ...
    if ANode.Children[i] <> ID_SOAP_INVALID then
      ...
      if (LNode.ReadingReferenceId <> 0) or (LNode.HRef <> '') then
        begin
        LParam := nil;
        if LNode.ReadingReferenceId <> 0 then
          ...
          begin
          if NameReferences.Find(LNode.HRef, LIndex) then
            begin
...
function TIdSoapReaderXML.ReadDimeWrapper(ASoapPacket: TStream; var VMimeType : String): TStream;
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ReadDimeWrapper';
var
...
begin
  ...
  try
    ...
    Result := LRec.Content;
    LRec.OwnsContent := false;
    if LRec.TypeType = dtMime then
      begin
      VMimeType := String(LRec.TypeInfo);
      end;
    for i := 1 to LDime.RecordCount - 1 do
      begin
      LRec := LDime.Item[i];
      LAtt := Attachments.Add(String(LRec.Id), false);
      LAtt.MediaType := '';
      case LRec.TypeType of
        dtMime : LAtt.MediaType := String(LRec.TypeInfo);
        dtURI  : LAtt.Headers.Values['URIType'] := String(LRec.TypeInfo);
      else
        {dtNotKnown, dtInvalid : }; // nothing
      end;
      LReplace := true;
      if assigned(OnCreateAttachment) then
        begin
        OnCreateAttachment(self, LAtt, LReplace);
        end;
      if LReplace then
        begin
        ...
        begin
        LAtt.Content.CopyFrom(LRec.Content, 0);
...
function TIdSoapReaderXML.ReadMimeWrapper(ASoapPacket: TStream; var VMimeType : String): TStream;
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ReadDimeWrapper';
var
  LMime : TIdSoapMimeMessage;
...
begin
  ...
  LMime := TIdSoapMimeMessage.create;
  try
    LMime.ReadFromStream(ASoapPacket, VMimeType);
    IdRequire(LMime.Parts.Count > 0, ASSERT_LOCATION+': No Parts found in MIME Message');
    LPart := LMime.MainPart;
    Result := LPart.Content;
    LPart.OwnsContent := false;
    VMimeType := LPart.MediaType;
    for i := 0 to LMime.Parts.Count - 1 do
      begin
      LPart := LMime.Parts.PartByIndex[i];
      if LPart.Id <> LMime.Start then
        begin
        LAtt := Attachments.Add(LPart.Id, false);
        LAtt.MediaType := LPart.MediaType;
        LAtt.Headers.Assign(LPart.Headers);
        LReplace := true;
        if assigned(OnCreateAttachment) then
          begin
          OnCreateAttachment(self, LAtt, LReplace);
        ...
        if LReplace then
          begin
          LAtt.Content := LPart.Content;
          LPart.OwnsContent := false;
          end
        else
          begin
          LAtt.Content.CopyFrom(LPart.Content, 0);
          LAtt.Content.Position := 0;
          end;
        end;
      end;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ResolveNamespace';
var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetXMLElement';
var
  LParam : TIdReaderParameter;
  LChild : TIdSoapNode;
begin
...
function TIdSoapReaderXML.GetParamIsAttribute(ANode: TIdSoapNode; const AName: String): Boolean;
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamIsAttribute';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapArrayDescription.ReadArrayInfo';
var
  LTemp : string;
  LSize : string;
  LJunk : string;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapArrayDescription.CheckFullyPopulated';
var
  i : integer;
begin
  assert(self.TestValid(TIdSoapArrayDescription), ASSERT_LOCATION+': self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapArrayDescription.GetNodeForNextItem';
var
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ReadArray';
var
  LId : string;
  LIdRef : integer;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ReadReference';
var
  LId : string;
  LIdRef : integer;
  LName : string;
...
begin
  ...
  if LId[1] = '#' then
    ...
    begin
    // this is a reference to an attachment - and therefore not an array (we guss)
    // we will treat it as a complex
    LType := AXmlElement.getAttribute(SchemaInstanceNamespace, ID_SOAP_NAME_SCHEMA_TYPE);
    if LType <> '' then
      begin
      SplitNamespace(LType, LTypeNS, LType);
      LTypeNS := AXmlElement.ResolveXMLNamespaceCode(LTypeNS, 'Type on Reference Node '+LName);
      end
    else
      begin
      LTypeNS := '';
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ReadElement';
var
  LType : string;
  LTypeNS : string;
  LRef : String;
begin
  ...
  if AXmlElement.hasAttribute('', ID_SOAP_NAME_XML_HREF) then
    begin
    // another gotcha. The entire infrastucture will be rewritten to resolve this soon
    // if the reference starts with a '#', then we consider this an internal array/object reference,
    // otherwise we consider it an external referece, and call it a parameter
    LRef := AXmlElement.getAttribute('', ID_SOAP_NAME_XML_HREF);
    if (LRef <> '') and (LRef[1] <> '#') then
      begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.ReadReferences';
var
  LTemp : string;
  LType : string;
  LTypeNS : string;
begin
  ...
  while assigned(AXmlElement) and (AXmlElement.hasID) and (EncodingMode = semRPC) do
    begin
    LTemp :=  AXmlElement.GetXSIType;
    if Pos(':', LTemp) > 0 then
      begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamExists';
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': self is not valid');
  if ANode = nil then
    begin
...
function TIdSoapReaderXML.GetParameter(ANode: TIdSoapNode; const AName: String; canBeNull : boolean = false): TIdReaderParameter;
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParameter';
var
  LIndex : integer;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': self is not valid');
  ...
  if LIndex < 0 then
    begin
    IdRequire(canBeNull or (seoUseDefaults in EncodingOptions), ASSERT_LOCATION+': '+Format(RS_ERR_SOAP_PARAM_MISSING, [AName, ANode.Params.CommaText]));
    result := nil;
    end
  else
    begin
...
procedure TIdSoapReaderXML.EnforceType(const AParamName: string; const AParam : TIdReaderParameter; const ANamespaceList : array of string; const ATypeList : array of string; ARoutineName: string); // clunky, but anamespace and aname iterate together
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.EnforceType';
var
  i : integer;
  LTypes : string;
  LFound : boolean;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamBoolean';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamByte';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamCardinal';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamChar';
var
  LString: String;
  LParam : TIdReaderParameter;
begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamWideChar';
var
  LString: WideString;
  LParam : TIdReaderParameter;
begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamComp';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamCurrency';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamDateTime';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamDouble';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamExtended';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamInt64';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamShortInt';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamShortString';
Var
  LParam : TIdReaderParameter;
  LString: String;
begin
  ...
  if assigned(LParam) and (not LParam.FIsNil) then
    ...
    if seoUseCrLf in EncodingOptions then
      begin
      LString := IdSoapAdjustLineBreaks(LString, tislbsCRLF);
      end;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamSingle';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamSmallInt';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamWideString';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamWord';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
function TIdSoapReaderXML.GetParamRef(ANode: TIdSoapNode; const AName, AType, ATypeNS: String): String;
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamWord';
Var
  LParam : TIdReaderParameter;
begin
  ...
  if assigned(LParam) and (not LParam.FIsNil) then
    ...
    if AType <> '' then
      begin
      EnforceType(AName, LParam,  [ATypeNS], [AType], ASSERT_LOCATION);
      end;
    Result := LParam.FHRef;
    end
  else
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamEnumeration';
var
  LParam : TIdReaderParameter;
  LEnumValue: String;
begin
  ...
  if assigned(LParam) and (not LParam.FIsNil) then
    ...
    if assigned(AItiLink) then
      begin
      LEnumValue := AItiLink.ReverseReplaceEnumName(string(ATypeInfo^.Name), LEnumValue);
      end;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamSet';
var
  LParam : TIdReaderParameter;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetGeneralParam';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
function TIdSoapReaderXML.GetHasEmptyParam(ANode: TIdSoapNode; const AName: String): Boolean;
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamIsAttribute';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
  LParam := GetParameter(ANode, AName, true);
  if assigned(LParam) and (LParam.FTypeVal = '') and (LParam.FValue = '') then
    Result := true
  else
    result := False;
end;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapReaderXML.GetParamBinary';
Var
  LParam : TIdReaderParameter;
begin
  assert(self.TestValid(TIdSoapReaderXML), ASSERT_LOCATION+': Self is not valid');
...
begin
  ...
  FDom := IdSoapDomFactory(XmlProvider);
  FDom.StartBuild(FNamespaceInfo.GetNameSpaceCode(ID_SOAP_NS_SOAPENV, DEF_OK)+ID_SOAP_NAME_ENV, ID_SOAP_NS_SOAPENV);
end;

destructor TIdSoapWriterXML.destroy;
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.destroy';
begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.ArrayIsSparse';
var
  LList : TStringList;
  i : integer;
begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.GetArrayDepth';
begin
  assert(Self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': self is not valid');
  assert(ASoapNode.TestValid(TIdSoapNode), ASSERT_LOCATION+': ASoapNode is not valid');
  assert(ASoapNode.IsArray, ASSERT_LOCATION+': not an array');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.EncodeArrayStraight';
var
  LElement : TIdSoapXmlElement;
  i : integer;
begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.EncodeArrayInLine';
var
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.EncodeArrayComplex';
var
  LElement : TIdSoapXmlElement;
  i : integer;
begin
  ...
  if ADepth = 1 then
    ...
    if AComplex then
      ...
      for i := 0 to ASoapNode.Children.Count - 1 do
        ...
        LElement := AParent.appendChild(ID_SOAP_NAME_SCHEMA_ITEM, '');
        LElement.setAttribute(ID_SOAP_NS_SOAPENC, FNamespaceInfo.GetNameSpaceCode(ID_SOAP_NS_SOAPENC, NO_DEF)+ID_SOAP_NAME_SCHEMA_POSITION, '['+CommaAdd(ACurrentLocation,ASoapNode.Children[i])+']');
        EncodeNode(LElement, ASoapNode.Children.Objects[i] as TIdSoapNode, '');
        end;
      end
    else
      ...
      for i := 0 to ASoapNode.Params.Count - 1 do
        ...
        Assert(ASoapNode.Params[i] <> '', ASSERT_LOCATION+': Name of '+inttostr(i)+'th parameter is blank');
        LElement := AParent.appendChild(ID_SOAP_NAME_SCHEMA_ITEM, '');
        LElement.setAttribute(ID_SOAP_NS_SOAPENC, FNamespaceInfo.GetNameSpaceCode(ID_SOAP_NS_SOAPENC, NO_DEF)+ID_SOAP_NAME_SCHEMA_POSITION, '['+CommaAdd(ACurrentLocation, ASoapNode.Params[i])+']');
        EncodeParam(ASoapNode.Params.objects[i] as TIdSoapWriterParameter, LElement, false);
        end;
      end;
    end
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.EncodeArray';
var
  LComplex : boolean;
  LDepth : integer;
  LSparse : boolean;
...
begin
  ...
  AParent.setAttribute(FNamespaceInfo.GetNameSpaceCode(SchemaInstanceNamespace, NO_DEF)+ID_SOAP_NAME_SCHEMA_TYPE, FNamespaceInfo.GetNameSpaceCode(ID_SOAP_NS_SOAPENC, NO_DEF)+ID_SOAP_SOAPENC_ARRAY, SchemaInstanceNamespace);
  ...
  if (EncodingMode = semRPC) and not (seoSuppressTypes in EncodingOptions) then
    begin
    AParent.setAttribute(ID_SOAP_NS_SOAPENC, FNamespaceInfo.GetNameSpaceCode(ID_SOAP_NS_SOAPENC, NO_DEF)+ID_SOAP_SOAPENC_ARRAYTYPE, LArrayType);
    end;
end;

function TIdSoapWriterXML.EncodeComplex(AParent: TIdSoapXmlElement; ASoapNode: TIdSoapNode; Namespace : String) : TIdSoapXmlElement;
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.EncodeComplex';
var
  i : integer;
  aParamInfo : TIdSoapWriterParameter;
begin
  assert(Self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': self is not valid');
  assert(FDom.TestValid(TIdSoapXMLDOM), ASSERT_LOCATION+': FDom is not valid');
  assert(AParent.TestValid(TIdSoapXmlElement), ASSERT_LOCATION+': AParent is not valid');
  ...
  if assigned(ASoapNode.Reference) then
    ...
    begin
    if (ASoapNode <> BaseNode) and ( {Header nodes} ASoapNode.Name <> ID_SOAP_NULL_NODE_NAME) then
      begin
      if (ASoapNode.ForceTypeInXML) or (not ((EncodingMode = semDocument) or (seoSuppressTypes in EncodingOptions))) then
        begin
        AParent.setAttribute(SchemaInstanceNamespace, FNamespaceInfo.GetNameSpaceCode(SchemaInstanceNamespace, NO_DEF)+ID_SOAP_NAME_SCHEMA_TYPE, FNamespaceInfo.GetNameSpaceCode(ASoapNode.TypeNamespace, NO_DEF)+ASoapNode.TypeName);
        end;
      end;
    if ASoapNode.HRef <> '' then
      begin
      AParent.setAttribute('', ID_SOAP_NAME_XML_HREF, ASoapnode.HRef);
      end;
    for i := 0 to ASoapNode.InOrder.count -1 do
      ...
      if ASoapNode.InOrder.objects[i] is TIdSoapWriterParameter then
        begin
        aParamInfo := ASoapNode.InOrder.objects[i] as TIdSoapWriterParameter;
        if (EncodingMode = semDocument) And AParamInfo.FIsAttribute And Not AParamInfo.FIsNil Then
        begin
          if aParamInfo.FUseWideString Then
            AParent.setAttribute('', ASoapNode.InOrder[i], aParamInfo.FValueW)
          else
            AParent.setAttribute('', ASoapNode.InOrder[i], aParamInfo.FValue);
        End
        Else
        begin
          result := AParent.appendChild(ASoapNode.InOrder[i], MessageNamespace);
          EncodeParam(aParamInfo, result, true);
        End
        end
      else
        begin
        result := AParent.appendChild(ASoapNode.InOrder[i], Namespace);
        EncodeNode(result, ASoapNode.InOrder.Objects[i] as TIdSoapNode, Namespace);
        end;
      end;
    end;
end;

procedure TIdSoapWriterXML.EncodeNode(AParent: TIdSoapXmlElement; ASoapNode: TIdSoapNode; Namespace : String);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.EncodeNode';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': self is not valid');
  ...
  if ASoapNode.IsArray then
    ...
    begin
    EncodeComplex(AParent, ASoapNode, Namespace);
    end;
end;

procedure TIdSoapWriterXML.EncodeReferenceObjects(AParent: TIdSoapXmlElement);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.EncodeReferenceObjects';
var
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.Encode';
var
  LBody: TIdSoapXmlElement;
...
begin
  ...
  LBody := FDom.Root.appendChild(FNamespaceInfo.GetNameSpaceCode(ID_SOAP_NS_SOAPENV, DEF_OK)+ID_SOAP_NAME_BODY, ID_SOAP_NS_SOAPENV);
  if ENcodingMode = semRPC then
    begin
    LMethod := LBody.appendChild(FNamespaceInfo.GetNameSpaceCode(MessageNamespace, DEF_OK)+MessageName, MessageNamespace);
    LMethod.setAttribute(ID_SOAP_NS_SOAPENV, FNamespaceInfo.GetNameSpaceCode(ID_SOAP_NS_SOAPENV, DEF_OK)+ID_SOAP_NAME_ENCODINGSTYLE, ID_SOAP_NS_SOAPENC);
    end
  else
    begin
    LMethod := LBody.appendChild(MessageName, MessageNamespace);
    FNamespaceInfo.DefineDefaultNamespace(MessageNamespace, LMethod);
    end;

  EncodeNode(LMethod, BaseNode, MessageNamespace);
  ...
  if Attachments.count > 0 then
    begin
    if AttachmentEncoding = iatDime then
      begin
      EncodeAttachmentsDime(AStream, VMimeType);
      end
    else
      begin
      EncodeAttachmentsMime(AStream, VMimeType);
      end;
    end;
end;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.AddArray';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  if ANode = NIL then
    ANode := BaseNode;
...
procedure TIdSoapWriterXML.DefineParameter(ANode: TIdSoapNode; const AName, AType, ATypeNamespace, AValue: String; AIsNil : boolean = false; AHRef : string = '');
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParameter';
var
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParameterW';
var
  LParamInfo : TIdSoapWriterParameter;
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
...
procedure TIdSoapWriterXML.DefineParamAnsiString(ANode: TIdSoapNode; const AName: String; AValue: AnsiString);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamAnsiString';
var
  LValue : String;
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  ...
  DefineParameter(ANode, AName, ID_SOAP_XSI_TYPE_STRING, SchemaNamespace, IdSoapAdjustLineBreaks(LValue, tislbsLF));
end;

procedure TIdSoapWriterXML.DefineParamString(ANode: TIdSoapNode; const AName, AValue: String);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamString';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  assert(AName <> '', ASSERT_LOCATION+': Name = ''''');
  // no check on AValue
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamInteger';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamBoolean';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamByte';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamCardinal';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamChar';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamWideChar';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamComp';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamCurrency';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamDateTime';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
  if AValue <> 0 then
    DefineParameter(ANode, AName, ID_SOAP_XSI_TYPE_DATETIME, SchemaNamespace, IdDateTimeToStr(AValue));
end;

procedure TIdSoapWriterXML.DefineParamDouble(ANode: TIdSoapNode; const AName: String; AValue: Double);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamDouble';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamExtended';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamInt64';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamShortInt';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamShortString';
var
  LValue : String;
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
  LValue := IdSoapAdjustLineBreaks(string(AValue), tislbsLF);
  if seoCheckStrings in EncodingOptions then
    begin
    CheckForBadCharacters(LValue, AName, ASSERT_LOCATION);
    end;
  DefineParameter(ANode, AName, ID_SOAP_XSI_TYPE_STRING, SchemaNamespace, LValue);
end;

procedure TIdSoapWriterXML.DefineParamSingle(ANode: TIdSoapNode; const AName: String; AValue: Single);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamSingle';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamSmallInt';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamWideString';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
  DefineParameterW(ANode, AName, ID_SOAP_XSI_TYPE_STRING, SchemaNamespace, AValue);
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamWord';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamXML';
var
  LParamInfo : TIdSoapWriterParameter;
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
...
procedure TIdSoapWriterXML.DefineParamRef(ANode: TIdSoapNode; const AName, AType, ATypeNS: String; AValue: String);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamRef';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  // DefineParameter will check parameters
  DefineParameter(ANode, AName, AType, ATypeNS, '', false, AValue);
end;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamEnumeration';
var
  LVal: String;
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  assert(ATypeInfo <> NIL, ASSERT_LOCATION+': ATypeInfo = nil');
  assert(IdEnumIsValid(ATypeInfo, AValue), ASSERT_LOCATION+': Enumeration Out of Range (' + IntToStr(AValue) + ':' + String(ATypeInfo.Name) + ')');
  // DefineParameter will check parameters

  LVal := IdEnumToString(ATypeInfo, AValue);
  Assert(LVal <> '', 'Unknown enumeration value [' + IntToStr(AValue) + '] in Enumeration Type [' + String(ATypeInfo^.Name) +']');
  if assigned(AItiLink) then
    begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamSet';
var
  LTypeData : PTypeData;
  i : integer;
  LTokens : String;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineParamBinary';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.GetMimeType';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.SetNilAttribute';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': Self is not valid');
  assert(AXmlElement.TestValid(TIdSoapXmlElement), ASSERT_LOCATION+': Element is not valid');
  // no check AValue
  ...
  if seoUseNullForNil in EncodingOptions then
    begin
    AXmlElement.setAttribute(SchemaInstanceNamespace, FNamespaceInfo.GetNameSpaceCode(SchemaInstanceNamespace, NO_DEF)+ID_SOAP_XSI_ATTR_NULL, BoolToXML(AValue));
    end
  else
    begin
    AXmlElement.setAttribute(SchemaInstanceNamespace, FNamespaceInfo.GetNameSpaceCode(SchemaInstanceNamespace, NO_DEF)+ID_SOAP_XSI_ATTR_NIL, BoolToXML(AValue));
    end;
end;

procedure TIdSoapWriterXML.WriteHeaders(ADocElement : TIdSoapXmlElement);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.CheckHeaders';
var
...
begin
  ...
  if FHeaders.Count > 0 then
    begin
    LHeaderElem := ADocElement.appendChild(FNamespaceInfo.GetNameSpaceCode(ID_SOAP_NS_SOAPENV, DEF_OK)+ID_SOAP_NAME_HEADER, ID_SOAP_NS_SOAPENV);
    ...
    try
      for i := 0 to FHeaders.count -1 do
        ...
        if assigned(LHeader.Node) then
          begin
          LElem := EncodeComplex(LHeaderElem, LHeader.Node, MessageNamespace);
          if assigned(LElem) and LHeader.MustUnderstand then
            begin
            LElem.setAttribute(ID_SOAP_NS_SOAPENV, FNamespaceInfo.GetNameSpaceCode(ID_SOAP_NS_SOAPENV, DEF_OK)+ ID_SOAP_NAME_MUSTUNDERSTAND, '1');
            end;
          end
        else if LHeader.MustSend or FHeaders.SendAll then
          begin
          idRequire(LHeader.Content is TIdSoapString, ASSERT_LOCATION+': A header that must be sent was encountered, but the header is not part of the method metadata, and it does not have simple content');
          LElem := LHeaderElem.appendChild(FNamespaceInfo.GetNameSpaceCode(LHeader.Namespace, DEF_OK)+LHeader.Name, LHeader.Namespace);
          Assert(LHeader.Content is TIdSoapString, ASSERT_LOCATION+': Header "{'+LHeader.Namespace+'}'+LHeader.Name+'" should contain String Content (Due to interface configuration), but contains '+LHeader.Content.ClassName);
...
procedure TIdSoapWriterXML.EncodeAttachmentsDime(AStream: TStream; var VMimeType : String);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.EncodeAttachments';
var
  LDime : TIdSoapDimeMessage;
  LRec : TIdSoapDimeRecord;
  LAtt : TIdSoapAttachment;
...
begin
  ...
  assert(VMimeType <> '', ASSERT_LOCATION+': Mime Type is not valid');
  ...
  try
    ...
    for i := 0 to Attachments.count -1 do
      ...
      LRec := LDime.Add(AsAnsiStringCheck(LAtt.Id, 'Mime Attachment Id'));
      if (LAtt.MediaType <> '') then
        begin
        LRec.TypeType := dtMime;
        LRec.TypeInfo := AsAnsiStringCheck(LAtt.MediaType, 'Mime Media Type');
        end
      else if LAtt.Headers.Values['URIType'] <> '' then
        begin
        LRec.TypeType := dtURI;
        LRec.TypeInfo := AsAnsiStringCheck(LAtt.Headers.Values['URIType'], 'Mime URI Type');
        end
      else
        begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineGeneralParam';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.DefineNamespace';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': self is not valid');
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.EncodeParam';
begin
  assert(self.TestValid(TIdSoapWriterXML), ASSERT_LOCATION+': self is not valid');
  ...
  if AIncludeType and (EncodingMode = semRPC) and not (seoSuppressTypes in EncodingOptions) then
    begin
    AElement.setAttribute(SchemaInstanceNamespace, FNamespaceInfo.GetNameSpaceCode(SchemaInstanceNamespace, NO_DEF)+ID_SOAP_NAME_SCHEMA_TYPE,  AParamInfo.FTypeNamespaceCode+AParamInfo.FType);
    end;
  if assigned(AParamInfo.FElem) then
    ...
    begin
    if AParamInfo.FHref <> '' then
      begin
      AElement.setAttribute('', ID_SOAP_NAME_XML_HREF, AParamInfo.FHref);
      end;
...
procedure TIdSoapWriterXML.EncodeAttachmentsMime(AStream: TStream; var VMimeType: String);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterXML.EncodeAttachmentsMime';
var
  LMsg : TIdSoapMimeMessage;
  LPart : TIdSoapMimePart;
  LAtt : TIdSoapAttachment;
  i, j : integer;
  s : String;
begin
  ...
  LMsg := TIdSoapMimeMessage.create;
  try
    LMsg.Start := ID_SOAP_MIME_DEFAULT_START;
    LMsg.MainType := VMimeType;
    LMsg.Boundary := ID_SOAP_MIME_DEFAULT_BOUNDARY;
    LPart := LMsg.Parts.AddPart(LMsg.Start);
    LPart.Content.CopyFrom(AStream, 0);
    LPart.Content.Position := 0;
    LPart.MediaType := VMimeType;
    LPart.TransferEncoding := 'binary';
    for i := 0 to Attachments.count -1 do
      begin
      LAtt := Attachments.AttachmentByIndex[i];
      LPart := LMsg.Parts.AddPart(LAtt.Id);
      if (LAtt.MediaType <> '') then
        begin
        LPart.MediaType := LAtt.MediaType;
        end;
      for j := 0 to LAtt.Headers.count -1 do
        begin
        s := LAtt.Headers.Names[j];
        if (not AnsiSameText(s, ID_SOAP_CONTENT_TYPE)) and (not AnsiSameText(s, ID_SOAP_MIME_ID)) then
          begin
          LPart.Headers.Values[s] := LAtt.Headers.Values[s];
          end;
        end;
      if LPart.TransferEncoding = '' then
        begin
        LPart.TransferEncoding := 'base64';
        end;
      LPart.Content := LAtt.Content;
      LPart.OwnsContent := false;
      end;
    AStream.Size := 0;
    LMsg.WriteToStream(AStream, false);
    VMimeType := LMsg.GetContentTypeHeader;
  finally
    FreeAndNil(LMsg);
  end;
end;

procedure TIdSoapWriterXML.DefineParamIsAttribute(ANode: TIdSoapNode; const AName: String; aValue : Boolean);
var
  iIndex : Integer;
  LParam : TIdSoapWriterParameter;
begin
  iIndex := ANode.Params.IndexOf(aName);
  if iIndex > -1 Then
  Begin
    LParam := TIdSoapWriterParameter(ANode.Params.Objects[iIndex]);
    LParam.FIsAttribute := aValue;
  End;
end;

{ TIdSoapFaultWriterXml }
procedure TIdSoapFaultWriterXml.BuildDetailsXML(ADom: TIdSoapXMLDOM; AFaultNode: TIdSoapXmlElement);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapFaultWriterXml.BuildDetailsXML';
var
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapFaultWriterXml.Encode';
var
  LDom: TIdSoapXMLDOM;
  LBody: TIdSoapXmlElement;
  LFault: TIdSoapXmlElement;
...
begin
  ...
  try
    LDom.StartBuild(ID_SOAP_NS_SOAPENV_CODE+':'+ID_SOAP_NAME_ENV, ID_SOAP_NS_SOAPENV);
    LDom.Root.setAttribute('http://www.w3.org/2000/xmlns/', ID_SOAP_NAME_XML_XMLNS+':'+ID_SOAP_NS_SOAPENV_CODE, ID_SOAP_NS_SOAPENV);
    LDom.Root.setAttribute('http://www.w3.org/2000/xmlns/', ID_SOAP_NAME_XML_XMLNS+':'+ID_SOAP_NS_SOAPENC_CODE, ID_SOAP_NS_SOAPENC);
    LDom.Root.setAttribute('http://www.w3.org/2000/xmlns/', ID_SOAP_NAME_XML_XMLNS+':'+ID_SOAP_NS_SCHEMA_CODE, ID_SOAP_NS_SCHEMA_2001);
    LDom.Root.setAttribute('http://www.w3.org/2000/xmlns/', ID_SOAP_NAME_XML_XMLNS+':'+ID_SOAP_NS_SCHEMA_INST_CODE, ID_SOAP_NS_SCHEMA_INST_2001);
    WriteHeaders(LDom, LDom.Root);
    LBody := LDom.Root.appendChild(ID_SOAP_NS_SOAPENV_CODE+':'+ID_SOAP_NAME_BODY, ID_SOAP_NS_SOAPENV);
    LFault := LBody.appendChild(ID_SOAP_NS_SOAPENV_CODE+':'+ID_SOAP_NAME_FAULT, ID_SOAP_NS_SOAPENV);

    if FCode <> '' then
      begin
      LElement := LFault.appendChild(ID_SOAP_NS_SOAPENV_CODE+':'+ID_SOAP_NAME_FAULTCODE, ID_SOAP_NS_SOAPENV);
    ...
    if FActor <> '' then
      begin
      LElement := LFault.appendChild(ID_SOAP_NS_SOAPENV_CODE+':'+ID_SOAP_NAME_FAULTACTOR, ID_SOAP_NS_SOAPENV);
      LElement.TextContentA := FActor;
      end;

    LElement := LFault.appendChild(ID_SOAP_NS_SOAPENV_CODE+':'+ID_SOAP_NAME_FAULTSTRING, ID_SOAP_NS_SOAPENV);
    ...
    if FDetails <> '' then
      ...
      begin
      LDetails := LFault.appendChild(ID_SOAP_NS_SOAPENV_CODE+':'+ID_SOAP_NAME_FAULTDETAIL, ID_SOAP_NS_SOAPENV);
      LDetails.setAttribute(ID_SOAP_NS_SCHEMA_INST_2001, ID_SOAP_NS_SCHEMA_INST_CODE+':'+ID_SOAP_NAME_SCHEMA_TYPE, FClass);
      end;
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapFaultWriterXml.GetMimeType';
begin
  assert(self.TestValid(TIdSoapFaultWriterXml), ASSERT_LOCATION+': self is not valid');
  if FUseUTF16 then
    begin
...
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapFaultWriterXml.CheckHeaders';
var
  LHeaderElem : TIdSoapXmlElement;
  i : integer;
  LElem : TIdSoapXmlElement;
