...
Type
  TIdSoapBasicType = ( isbtBoolean,
                       ...
                       isbtShortString, isbtLongString, isbtWideString, isbtUnicodeString,
...
begin
  ...
  case ATypeInfo^.Kind of
    ...
    {$IFDEF UNICODE}
    tkUString:        result := isbtUnicodeString;
    {$ENDIF}
    ...
    tkEnumeration:
      begin
      if AnsiSameText(String(ATypeInfo^.Name), 'Boolean') then { do not localize } // kind of a special case as its not really a simple type but we treat it like one
        begin
        result := isbtBoolean;
        end
      else
...
begin
  case ABasicType of
    ...
    {$IFDEF UNICODE}
    isbtUnicodeString:  result := sizeof(pointer);
    {$ENDIF}
...
begin
  case ABasicType of
    ...
    {$IFDEF UNICODE}
    isbtUnicodeString:  result := 4;
    {$ENDIF}
    isbtWideString:     result := 4;
    isbtDynArray:       result := 4;
    isbtClass:          result := 4;
    else
      raise EIdSoapRequirementFail.create(ASSERT_LOCATION+': Unknown Basic type '+inttostr(ord(ABasicType)));
    end;
end;
...
begin
  case ABasicType of
    ...
    {$IFDEF UNICODE}
    isbtUnicodeString:     result := 'UnicodeString';
    {$ENDIF}
