...
const
  CURRENT_BINARY_VERSION = {$IFDEF UNICODE} 2 {$ELSE} 1 {$ENDIF}; // cause strings changed...
...
begin
  ...
  case LParamType of
    ...
    ID_SOAP_BIN_TYPE_SHORTSTRING    : StreamSkip(FStream, StreamReadByte(FStream) {$IFDEF UNICODE}*2{$ENDIF});
    ...
    ID_SOAP_BIN_TYPE_SET            :
      begin
      StreamReadLongString(FStream);
      StreamReadLongString(FStream);
      StreamSkip(FStream, Sizeof(Integer));
      end;
...
begin
  ...
  case LNodeType of
    ID_SOAP_BIN_NODE_STRUCT:
      begin
      LClass := StreamReadLongString(FStream);
      LClassNS := StreamReadLongString(FStream);
  ...
  if Assigned(ANode) then
    begin
    Assert(ANode.Children.IndexOfName(string(LName)) = -1, ASSERT_LOCATION+': Name "'+LName+'" used more than once in Packet');
    ANode.Children.AddObject(String(LName), LNode);
...
begin
  ...
  if LPacketType = ID_SOAP_BIN_PACKET_EXCEPTION then
    ...
    StreamReadBinary(FStream, @LNamespace[1], length(LNameSpace){$IFDEF UNICODE}*2{$ENDIF});
    MessageNameSpace := LNamespace;
    MessageName := StreamReadLongString(FStream);
    end;
end;
...
begin
  ...
  IdRequire(StreamReadLongString(FStream) = ID_SOAP_NULL_NODE_NAME, ASSERT_LOCATION+': Encountered a Bad Stream (First Node Name Wrong)');
  DecodeNode(BaseNode);
...
begin
  ...
  LName := StreamReadLongString(FStream);
  LNameNS := StreamReadLongString(FStream);
...
begin
  ...
  if seoUseCrLf in EncodingOptions then
    begin
    result := ShortString(IdSoapAdjustLineBreaks(String(result), tislbsCRLF));
    end;
end;
...
function TIdSoapReaderBin.GetHasEmptyParam(ANode: TIdSoapNode; const AName: String): Boolean;
begin
  result := false;  // doesn't arise in this context
end;
...
begin
  ...
  FStream.Position := GetParamPosition(ANode, AName);
  LType := StreamReadByte(FStream);
  Assert((LType = ID_SOAP_BIN_TYPE_BINARY) or (LType = ID_SOAP_BIN_TYPE_GENERAL), ASSERT_LOCATION+': Value is actually '+DescribeParamType(LType));
  if LType = ID_SOAP_BIN_TYPE_GENERAL then
  begin
    result := nil; // only call this if it it's nil
  end
  else
  begin
    LType := StreamReadByte(FStream);
...
begin
  ...
  for i := 0 to t - 1 do
    ...
    LAttach.MediaType := StreamReadLongString(FStream);
    if assigned(OnCreateAttachment) then
      begin
      OnCreateAttachment(self, LAttach, LDummy);
      end;
...
function TIdSoapReaderBin.GetParamRef(ANode: TIdSoapNode; const AName, AType, ATypeNS: String): String;
begin
  result := GetParamString(ANode, AName);
end;

function TIdSoapReaderBin.GetParamIsAttribute(ANode: TIdSoapNode; const AName: String): Boolean;
begin
  result := False; // because we don't care here
end;
...
begin
  ...
  StreamWriteLongString(BaseNode.Stream, ID_SOAP_NULL_NODE_NAME);
  FNamespaces := TStringList.create;
  FNamespaces.sorted := false; { index is identifier }
end;
...
begin
  ...
  StreamWriteLongString(Result.Stream, AName);
  StreamWriteByte(Result.Stream, ID_SOAP_BIN_NODE_ARRAY);
  StreamWriteLongString(Result.Stream, ABaseType);
  StreamWriteLongString(Result.Stream, ABaseTypeNS);
...
begin
  ...
  StreamWriteLongString(ANode.Stream, ANameSpace);
...
begin
  ...
  {$ENDIF}
  // no check on AValue
  AValue := ShortString(IdSoapAdjustLineBreaks(string(AValue), tislbsLF));
  if seoCheckStrings in EncodingOptions then
    begin
    CheckForBadCharacters(string(AValue), AName, ASSERT_LOCATION);
    end;
  StreamWriteByte(ANode.Stream, ID_SOAP_BIN_TYPE_PARAM);
  StreamWriteLongString(ANode.Stream, AName);
...
begin
  ...
  if length(LValue) > 0 then
    begin
    StreamWriteBinary(ANode.Stream, @LValue[1], Length(LValue) {$IFDEF UNICODE}*2{$ENDIF});
    end;
end;
...
begin
  ...
  StreamWriteLongString(AStream, MessageName);
  // start node list
  StreamWriteByte(AStream, ID_SOAP_BIN_TYPE_NODE);
...
begin
  ...
  StreamWriteLongString(ANode.Stream, ANode.Name);
  if ANode.Reference <> nil then
    ...
    if (EncodingMode = semRPC) and (not (seoSuppressTypes in EncodingOptions)) then
      begin
      StreamWriteLongString(ANode.Stream, ANode.TypeName);
      StreamWriteLongString(ANode.Stream, ANode.TypeNamespace);
      end
    else
      begin
      StreamWriteLongString(ANode.Stream, '');
      StreamWriteLongString(ANode.Stream, '');
      end;
...
begin
  ...
  for i := 0 to Attachments.Count - 1 do
    ...
    StreamWriteLongString(AStream, LAttach.MediaType);
    LAttach.Content.position := 0;
...
begin
  ...
  StreamWriteLongString(ANode.Stream, ATypeName);
  LStream := TIdMemoryStream.createString(AXML.AsXML);
  try
...
procedure TIdSoapWriterBin.DefineParamRef(ANode: TIdSoapNode; const AName, AType, ATypeNS: String; AValue: String);
begin
  DefineParamString(ANode, AName, AValue);
end;

procedure TIdSoapWriterBin.DefineParamIsAttribute(ANode: TIdSoapNode; const AName: String; aValue : Boolean);
begin
  // because it doesn't make any difference
end;

procedure TIdSoapWriterBin.DefineParamAnsiString(ANode: TIdSoapNode; const AName: String; AValue: AnsiString);
const ASSERT_LOCATION = ASSERT_UNIT+'.TIdSoapWriterBin.DefineParamString';
var
  LValue : AnsiString;
begin
  ...
  {$ENDIF}
  // no check on AValue
  LValue := AnsiString(IdSoapAdjustLineBreaks(String(AValue), tislbsLF));
  if seoCheckStrings in EncodingOptions then
    begin
    CheckForBadCharacters(String(LValue), AName, ASSERT_LOCATION);
    end;
  StreamWriteByte(ANode.Stream, ID_SOAP_BIN_TYPE_PARAM);
  StreamWriteLongString(ANode.Stream, AName);
...
begin
  ...
  StreamWriteLongString(AStream, FClass);
