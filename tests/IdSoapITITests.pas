...
begin
  ...
  Check(FIntf.UnitName_ = 'testunit', 'Unitname Feedback test failed');
end;
...
begin
  ...
  check(assigned(FIntf.FindMethodByName('TestMethRequest', ntMessageRequest)));
  check(assigned(FIntf.FindMethodByName('TestMeth1Request', ntMessageRequest)));
