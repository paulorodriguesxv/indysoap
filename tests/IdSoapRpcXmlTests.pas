...
{$ENDIF}
  ...
  Published
    ...
    procedure Test_IdSoap_Param_CharA;
...
{$ENDIF}
  ...
  TIdSoapMimeTests = class (TTestCase)
  Published
    procedure TestMime1;
    procedure TestMime2;
    procedure TestMime3;
    procedure TestMime4;
    procedure TestMimeHL7;
  end;

  TIdSoapPacketAttachmentTests = class(TTestCase)
  Private
    FStream: TIdMemoryStream;
    FWriter: TIdSoapWriter;
    FReader: TIdSoapReader;
  Protected
    ...
    procedure TearDown; Override;
  Published
    procedure Test_Dime_Attachment;
    procedure Test_Dime_AttachmentEmpty;
    procedure Test_Dime_2Attachments;
    procedure Test_Mime_Attachment;
    procedure Test_Mime_AttachmentEmpty;
    procedure Test_Mime_2Attachments;
  end;
...
begin
  FStream := TIdMemoryStream.Create;
...
begin
  ...
  FWriter.Encode(FStream, LMediaType, nil, nil);
end;
...
begin
  ...
  FWriter.Encode(FStream, LMediaType, nil, nil);
  FStream.Position := 0;
  FReader.ReadMessage(FStream, LMediaType, nil, nil);
...
begin
  ...
  FStream.Position := 0;
  {$IFNDEF UNICODE}
  FStream.SaveToFile('c:\temp\packet-D5.xml');
  {$ELSE}
  FStream.SaveToFile('c:\temp\packet-XE3.xml');
  {$ENDIF}
  FStream.Position := 0;
  FReader.ReadMessage(FStream, LMediaType, nil, nil);
  FReader.checkpacketOK;
...
begin
  ...
  if FReader.XmlProvider <> xpCustom then
    begin
    // Custom doesn't support widechar in non-unicode Delphi
  {$ENDIF}
...
procedure TIdSoapPacketBaseTests.Test_IdSoap_Param_CharA;
var
  LMediaType : string;
begin
  FWriter.SetMessageName('TestProc', 'urn:test');
  FWriter.DefineParamChar(NIL, 'AChar', 'A');
  FWriter.Encode(FStream, LMediaType, nil, nil);
  FStream.Position := 0;
  FReader.ReadMessage(FStream, LMediaType, nil, nil);
  FReader.checkpacketOK;
  FReader.ProcessHeaders;
  FReader.DecodeMessage;
  Check(FReader.ParamChar[NIL, 'AChar'] = 'A', 'Char A failed');
end;
...
begin
  ...
  FWriter.Encode(FStream, LMediaType, nil, nil);
  FStream.Position := 0;
  FStream.SaveToFile('c:\temp\soap.xml');
  FStream.Position := 0;
  FReader.ReadMessage(FStream, LMediaType, nil, nil);
  FReader.checkpacketOK;
  FReader.ProcessHeaders;
...
begin
  ...
  try
    ...
    FFault.Encode(FStream, LMediaType, nil, nil);
  finally
...
begin
  ...
  try
    ...
    Exception do
      begin
      Check(e.message = 'soap:Server: Testing', 'Exception message is wrong: should be "soap:Server: Testing" but is "'+e.message+'"');
      end
    end;
end;
...
procedure TIdSoapPacketAttachmentTests.Test_Dime_Attachment;
var
  LStream : TMemoryStream;
  ...
  LMediaType : string;
begin
  FWriter.AttachmentEncoding := iatDime;
  LStream := TIdMemoryStream.create;
  try
    FillTestingStream(LStream, 2000);
    FWriter.SetMessageName('TestProc', 'urn:test');
    LAtt := FWriter.Attachments.Add('Test');
    LAtt.MediaType := 'application/octet-stream';
    ...
    Check(LAtt.Id = 'Test');
    Check(LAtt.MediaType = 'application/octet-stream');
    LStream.Position := 0;
...
procedure TIdSoapPacketAttachmentTests.Test_Dime_2Attachments;
var
...
begin
  ...
  try
    ...
    LAtt := FWriter.Attachments.Add('sdff', false);
//    LAtt.URIType := 'asdasd';   //this will be suppressed - we can't deal with this for te moment
...
procedure TIdSoapPacketAttachmentTests.Test_Dime_AttachmentEmpty;
var
  LStream : TMemoryStream;
  LAtt : TIdSoapAttachment;
  LMsg : String;
...
procedure TIdSoapPacketAttachmentTests.Test_Mime_Attachment;
var
  LStream : TMemoryStream;
  LAtt : TIdSoapAttachment;
  LMsg : String;
  ...
  LMediaType : string;
begin
  FWriter.AttachmentEncoding := iatMime;
  LStream := TIdMemoryStream.create;
  try
    FillTestingStream(LStream, 2000);
    FWriter.SetMessageName('TestProc', 'urn:test');
...
procedure TIdSoapPacketAttachmentTests.Test_Mime_2Attachments;
var
  LStream1 : TMemoryStream;
  LStream2 : TMemoryStream;
  LAtt : TIdSoapAttachment;
...
procedure TIdSoapPacketAttachmentTests.Test_Mime_AttachmentEmpty;
var
  LStream : TMemoryStream;
  LAtt : TIdSoapAttachment;
  LMsg : String;
...
begin
  ...
  check(FDom.Root.FindElementAnyNS('TDPDECurrency').FindElementAnyNS('symbol').TextContentA = GetTargetStr(char(163)), ' character fail: found '+FDom.Root.FindElementAnyNS('TDPDECurrency').FindElementAnyNS('symbol').TextContentA +' expecting '+GetTargetStr(char(163)));
end;
...
procedure TIdSoapPacketAttachmentTests.SetUp;
begin
  FStream := TIdMemoryStream.Create;
  FWriter := TIdSoapWriterXML.create(IdSoapV1_1, xpCustom);
  FWriter.EncodingOptions := [seoUseCrLf, seoSuppressTypes, seoCheckStrings];
  FReader := TIdSoapReaderXML.create(IdSoapV1_1, xpCustom);
  FReader.EncodingOptions := [seoUseCrLf, seoCheckStrings];
end;

procedure TIdSoapPacketAttachmentTests.TearDown;
begin
  FreeAndNil(FStream);
  FreeAndNil(FWriter);
  FreeAndNil(FReader);
...
procedure TIdSoapXMLCustomTests.TestAddress;
begin
  LoadFile('XMLSamples'+PathDelim+'address.xml');
end;

{ TIdSoapMimeTests }

procedure TIdSoapMimeTests.TestMime1;
var
  LFile : TFileStream;
...
begin
  ...
  try
    LFile := TFileStream.create('Mime/sample1.mime', fmOpenRead or fmShareDenyWrite);
    try
      LMsg.ReadFromStream(LFile);
      check(LFile.Position = LFile.Size);
    finally
      FreeAndNil(LFile);
    end;
    check(LMsg.Parts.Count = 2);
    check(LMsg.Parts.PartByIndex[0].Id = 'http://claiming-it.com/claim061400a.xml');
    check(LMsg.MainPart.Id = 'http://claiming-it.com/claim061400a.xml');
    check(LMsg.Parts.PartByIndex[1].Id = 'http://claiming-it.com/claim061400a.tiff');
    LFile := TFileStream.create('Mime/sample1.mime.part1', fmOpenRead or fmShareDenyWrite);
    try
      if not TestStreamsIdentical(LFile, LMsg.MainPart.Content, LErr) then
        begin
        IdSoapShowStreamDiff(LFile, 'file', LMsg.MainPart.Content, 'part', 'txt', true);
        check(false, LErr);
        end;
    finally
      FreeAndNil(LFile);
    end;
    LFile := TFileStream.create('Mime/sample1.mime.part2', fmOpenRead or fmShareDenyWrite);
    try
      if not TestStreamsIdentical(LFile, LMsg.Parts.PartByIndex[1].Content, LErr) then
        begin
        check(false, LErr);
        end;
    finally
...
procedure TIdSoapMimeTests.TestMime2;
var
  LFile : TFileStream;
  LMsg : TIdSoapMimeMessage;
begin
  LMsg := TIdSoapMimeMessage.create;
  try
    LFile := TFileStream.create('Mime/sample2.mime', fmOpenRead or fmShareDenyWrite);
    try
      LMsg.ReadFromStream(LFile);
      check(LFile.Position = LFile.Size-1);
    finally
      FreeAndNil(LFile);
    end;
    check(LMsg.Parts.Count = 3);
    check(LMsg.MainPart.Id = 'claim061400a.xml@claiming-it.com');
    check(LMsg.Parts.PartByIndex[1].Id = 'claim061400a.tiff@claiming-it.com');
    check(LMsg.Parts.PartByIndex[2].Id = 'claim061400a.jpeg@claiming-it.com');
  finally
    FreeAndNil(LMsg);
  end;
end;

procedure TIdSoapMimeTests.TestMime3;
var
  LFile : TFileStream;
  LMsg : TIdSoapMimeMessage;
  LErr : String;
begin
  LMsg := TIdSoapMimeMessage.create;
  try
    LFile := TFileStream.create('Mime/sample3.mime', fmOpenRead or fmShareDenyWrite);
    try
      LMsg.ReadFromStream(LFile);
      check(LFile.Position = LFile.Size-4);
    finally
      FreeAndNil(LFile);
    end;
    check(LMsg.Parts.Count = 2);
    check(LMsg.Parts.PartByIndex[0].Id = '950120.aaCC@XIson.com');
    check(LMsg.MainPart.Id = '950120.aaCC@XIson.com');
    check(LMsg.Parts.PartByIndex[1].Id = '950121.aaCB@XIson.com');
    LFile := TFileStream.create('Mime/sample3.out.mime', fmCreate);
    try
      LMsg.WriteToStream(LFile, true);
    finally
      FreeAndNil(LFile);
    end;
    LFile := TFileStream.create('Mime/sample3.mime.part1', fmOpenRead or fmShareDenyWrite);
    try
      if not TestStreamsIdentical(LFile, LMsg.MainPart.Content, LErr) then
        begin
        check(false, LErr);
    ...
    LFile := TFileStream.create('Mime/sample3.mime.part2', fmOpenRead or fmShareDenyWrite);
    try
      if not TestStreamsIdentical(LFile, LMsg.Parts.PartByIndex[1].Content, LErr) then
        begin
        IdSoapShowStreamDiff(LFile, 'file', LMsg.Parts.PartByIndex[1].Content, 'part', 'txt', true);
        check(false, LErr);
        end;
    finally
      FreeAndNil(LFile);
...
procedure TIdSoapMimeTests.TestMime4;
var
  LFile : TFileStream;
  LMsg : TIdSoapMimeMessage;
begin
  LMsg := TIdSoapMimeMessage.create;
  try
    LFile := TFileStream.create('Mime/sample4.mime', fmOpenRead or fmShareDenyWrite);
    try
      LMsg.ReadFromStream(LFile);
      check(LFile.Position = LFile.Size-2);
    finally
      FreeAndNil(LFile);
    end;
    check(LMsg.Parts.Count = 3);
    check(LMsg.MainPart.Id = '950118.AEBH@XIson.com');
    check(LMsg.Parts.PartByIndex[1].Id = '950118.AFDH@XIson.com');
    check(LMsg.Parts.PartByIndex[2].Id = '950118.AECB@XIson.com');
  finally
    FreeAndNil(LMsg);
  end;
end;

procedure TIdSoapMimeTests.TestMimeHL7;
var
  LFile : TFileStream;
  LMsg : TIdSoapMimeMessage;
begin
  LMsg := TIdSoapMimeMessage.create;
  try
    LFile := TFileStream.create('Mime/hl7_ebxml.mime', fmOpenRead or fmShareDenyWrite);
    try
      LMsg.ReadFromStream(LFile);
      check(LFile.Position = LFile.Size-2);
    finally
    ...
    check(LMsg.Parts.Count = 2);
    check(LMsg.MainPart.Id = 'ebhmheader@example.com');
    check(LMsg.Parts.PartByIndex[1].Id = 'ebxmlpayload01@example.com');
    LMsg.Headers.Values['SOAPAction'] := '"ebXML"';
  finally
    FreeAndNil(LMsg);
  end;
end;
